...
Uma MV pode ser visualizada como uma camada utilizada para prover a integração entre duas interfaces (possivelmente distintas). Esta camada pode ser implementada com emulação em software, virtualização em hardware ou uma composição das duas abordagens.
...
em específico o uso de uma MV de sistema agrega ao \textit{overhead} de emulação da aplicação também o \textit{overhead} de emulação destes 
...
Nesse texto apresentamos uma proposta de avaliação do \textit{overhead} de emulação do sistema operacional e dispositivos de hardware
...
a transformação de uma máquina virtual de sistema em uma máquina virtual de processo. Além de uma melhor compreensão do \textit{overhead}
...
\begin{itemize}
	...
	controle dos periféricos do sistema (\textit{system-ISA}). Uma máquina virtual de sistema portanto deve emular tanto os dispositivos
	...
	define diversos padrões, os principais são: um subconjunto da ISA que é visível aos programas de usuário (\textit{user-ISA}) e uma 
	...
	Em sistemas operacionais \textit{Unix-like}, esta interface com o sistema operacional é através de \emph{system calls} (syscalls). 
...
porém, uma vez traduzido, o código resultante é executado nativamente, o que garante melhor desempenho. Sistemas
...
bibliotecas compartilhadas, códigos objetos, etc. utilizados em ambientes \textit{Unix-like}. Um arquivo ELF pode
...
\begin{itemize}
 	...
 	um executável pelo \textit{linker} dinâmico e outros objetos compartilhados para a criação de uma imagem de processo.
\end{itemize}
...
O sistema será composto de seis módulos principais que serão gerenciador pelo \textit{runtime}
