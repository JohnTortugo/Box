...
\subsubsection{Cache de Instruções}

A implementação da cache de instruções do Bochs, além de armazenar as instruções
recentemente executadas, fornece um conjunto de otimizações voltadas a reduzir o
impacto causado pela decodificação contínua de instruções. Para isso, são
implementados mecanismos de \emph{prefetch} e pré-decodificação das instrução e
formação de blocos. Em sua implementação padrão, a cache possui um total de 64K
entradas, onde cada entrada é representada por uma estrutura que contém o
endereço da estrutura que representa a primeira instrução decodificada e o
número de instruções no bloco.

Durante a execução da CPU virtual, o laço de interpretação solicita à cache a
próxima instrução a ser executada. Neste ponto, a cache verifica se a instrução
já foi decodificada anteriormente. Em caso afirmativo, o endereço da entrada no
cache que contém a instrução é devolvido ao laço. Caso a instrução não tenha
sido decodificada anteriormente, a cache faz o \emph{fetch} da página de memória
onde a instrução a ser executada está contida e inicia a decodificação de um
bloco de instruções que pode conter até 32 instruções. Através desse processo, o
Bochs reduz o tempo gasto com chamadas à rotina de decodificação de instruções.

Com as informações contidas na entrada de cache, o laço executa sequencialmente
todas as instruções existentes no bloco simplesmente incrementando o endereço da
instrução. Uma vez que o laço executa a última instrução do bloco, uma nova
consulta a cache ocorre, o que resulta em um novo ciclo de pré-decodificação.
...
aplicativos do Mibench executados nativamente, no Bochs e no Box sem nenhum tipo
de otimização. De modo geral, a comparação entre os tempos de execução das
máquinas virtuais de sistema e processo mostrou que a versão de sistema
apresentou tempos significativamente menores. Tal diferença pode ser justificada
pelo fato do Bochs contar com otimizações que são direcionadas à implementação
do subsistema de memória e a utilização da cache de instruções pré-decodificadas. Os resultados indicam que essas otimizações foram suficientes
para superar qualquer redução de tempo obtido pela eliminação de emulação da
parte de sistema operacional.

\begin{figure}[h]
	\centering
	\includegraphics[width=1.0\textwidth]{figures/bochs_nocache_nativo.pdf}
...
implementada a técnica DICache \cite{dicache}, na segunda, o código de pré-decodificação de instruções do Bochs foi incluído no Box. Para isso modificações
tiveram que ser feitas no código original do Bochs para garantir a
...
\begin{figure}[h]
	\centering
	\includegraphics[width=1.0\textwidth]{figures/dicache_nocache.pdf}
...
\begin{figure}[h]
        \centering
        \includegraphics[width=1.0\textwidth]{figures/bochs_icache.pdf}
...
de sistema Bochs.  Os resultados obtidos nos testes não sustentaram a hipótese
da existência de \emph{overhead} causado pela interpretação de todo o sistema
operacional em máquinas virtuais de sistema. Tal fato pode ser justificado pelos
seguintes fatores:
...
\begin{itemize}
 ...
 \item Em condições normais, a execução de um sistema operacional deve produzir
   um baixo impacto no tempo de execução da aplicação, uma vez que na maior
   parte do tempo o código em execução é da aplicação, e não do sistema
   operacional.
\end{itemize}
...
aplicação de técnicas de otimização em uma MV. A redução no tempo de execução
das aplicações executada no Box utilizando otimizações mostra o quanto a
interpretação pode ser beneficiada com a utilização de técnicas de otimização
como DICache e pré-decodificação.
...
\begin{itemize}
	\item Aprimorar o subsistema de memória, melhorando o controle de
          segmentação e fornecendo suporte a paginação de memória.
        \item Implementação de suporte a execução carregadores externos (não
          pertencente ao Box).
	...
	\item Suporte a execução em diferentes plataformas e ou sistemas
          operacionais.
\end{itemize}
...
\begin{table}[!h]
 ...
 \begin{tabular}{|l|r|r|r|r|r|r|}
   \cline{4-6}
   \multicolumn{3}{c|}{}& \multicolumn{3}{|c|}{Box} \\ \hline
   \bf{Benchmark} & \bf{Nativo} & \bf{Bochs} &  \bf{NoCache} & \bf{DICache} & \bf{ICache} \\ \hline
    basicmath & 1.023 & 58.402 & 233.463 & 179.211 & 74.511 \\ \hline
    bitcnts & 0.183 & 8.277 & 53.376 & 23.985 & 9.332 \\ \hline
    qsort & 0.25 & 16.224 & 68.704 & 46.879 & 20.299 \\ \hline
    susan-smoothing & 0.134 & 11.175 & 3.374 & 1.617 & 0.983 \\ \hline
    susan-edges & 0.037 & 1.904 & 9.182 & 7.627 & 2.558 \\ \hline
    susan-corners & 0.012 & 0.653 & 2.823 & 2.044 & 0.803 \\ \hline
    tiff2rgba & 0.335 & 8.053 & 16.606 & 7.887 & 4.701 \\ \hline
    tiffmedian & 0.278 & 12.389 & 50.93 & 25.397 & 13.447 \\ \hline
    tiffdither & 0.28 & 18.348 & 95.966 & 57.333 & 21.579 \\ \hline
    tiff2bw & 0.085 & 4.331 & 15.562 & 6.925 & 3.713 \\ \hline
    cjpeg & 0.05 & 2.003 & 9.294 & 4.845 & 2.525 \\ \hline
    djpeg & 0.017 & 0.777 & 2.964 & 1.746 & 0.921 \\ \hline
    patricia & 0.222 & 13.23 & 54.226 & 44.347 & 17.006 \\ \hline
    dijkstra & 0.057 & 2.809 & 15.967 & 7.686 & 2.968 \\ \hline
    ispell & 0.301 & 18.511 & 81.081 & 60.786 & 24.859 \\ \hline
    search & 0.01 & 0.238 & 0.859 & 0.636 & 0.279 \\ \hline
    say & 0.823 & 49.562 & 14.113 & 12.151 & 6.946 \\ \hline
    blowfish-e & 0.196 & 12.952 & 57.234 & 44.264 & 15.022 \\ \hline
    blowfish-d & 0.197 & 13.045 & 58.386 & 44.707 & 15.157 \\ \hline
    rijndael-e & 0.266 & 14.557 & 29.349 & 36.451 & 9.246 \\ \hline
    rijndael-d & 0.254 & 14.636 & 30.866 & 37.343 & 9.442 \\ \hline
    sha & 0.042 & 2.571 & 13.453 & 7.723 & 3.602 \\ \hline
    pgp-sa & 0.051 & 0.231 & 0.67 & 0.447 & 0.286 \\ \hline
    pgp-z & 0.039 & 0.361 & 1.22 & 0.714 & 0.445 \\ \hline
    rawcaudio & 0.339 & 12.855 & 72.592 & 33.437 & 15.301 \\ \hline
    rawdaudio & 0.279 & 10.383 & 59.891 & 27.648 & 13.387 \\ \hline
    crc & 0.305 & 20.076 & 95.995 & 50.293 & 27.88 \\ \hline
    toast & 0.501 & 34.731 & 184.047 & 110.08 & 38.157 \\ \hline
    untoast & 0.228 & 14.685 & 71.222 & 33.199 & 14.171 \\ \hline
  \end{tabular}
