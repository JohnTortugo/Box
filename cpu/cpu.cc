/////////////////////////////////////////////////////////////////////////
// $Id: cpu.cc 11356 2012-08-21 19:58:41Z sshwarts $
/////////////////////////////////////////////////////////////////////////
//
    ...
    */
    ...
    for(;;) {
      ...
      BX_INSTR_BEFORE_EXECUTION(BX_CPU_ID, i);
        ...
        printf("EIP: 0x%08lx POS: 0x%08lx\t", RIP, bx_mem.virtualAddressToPosition(RIP));
        printf("iLen: %02d \t Exec1: %0x \t\t", i->ilen(), i->execute);
        d.disasm32((bx_address)0, (bx_address)RIP, (const Bit8u *)bx_mem.VirtualToRealAddress(RIP), disbuf);
        printf("%s\n", disbuf);
        fflush(stdout);
#endif
...
void BX_CPU_C::prefetch(void)
{
  ...
  if ((tlbEntry->lpf == lpf) && !(tlbEntry->accessBits & (0x4 | USER_PL))) {
    ...
    BX_DEBUG(("B"));
    BX_CPU_THIS_PTR pAddrFetchPage = PPFOf(laddr);
  }
  ...
  if (fetchPtr) {
    ...
    BX_DEBUG(("D"));
    BX_DEBUG(("RIP: 0x%08lx pAddrFetchPage: 0x%08lx", RIP, pAddrFetchPage));
//    BX_CPU_THIS_PTR eipFetchPtr = (const Bit8u*) getHostMemAddr(BX_CPU_THIS_PTR pAddrFetchPage, BX_EXECUTE);
    BX_CPU_THIS_PTR eipFetchPtr = (const Bit8u*) BX_CPU_THIS_PTR pAddrFetchPage;
